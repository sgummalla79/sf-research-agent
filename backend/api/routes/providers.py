...
from utils.api_keys import encrypt, decrypt, populate_cache, is_configured
...
async def connect_provider(provider_id: str, payload: ConnectPayload, request: Request) -> dict:
    """Validate + save key, then fetch and cache the model list."""
    ...
    # Fetch models — this implicitly validates the key
    try:
...
async def disconnect_provider(provider_id: str, request: Request) -> dict:
    """Remove the API key and cached model list for a provider."""
    if provider_id not in PROVIDERS:
        raise HTTPException(status_code=404, detail=f"Unknown provider: {provider_id!r}")
